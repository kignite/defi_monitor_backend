...
try:  # Avoid runtime circular imports; only used for type checking
    from typing import TYPE_CHECKING
    if TYPE_CHECKING:  # pragma: no cover
        from defi_monitor import VaultConfig, UserConfig
except Exception:  # pragma: no cover - safety net if typing import is blocked
    TYPE_CHECKING = False
...
class VaultAdapter(Protocol):
    """
    Adapter interface so different protocols can plug in their own logic.

    Implementations should return a dict with the shape:
